...
class TestRenderFootings3D:
    ...
    def test_default_uses_turntable_with_z_up(self):
        """Default 3D view uses turntable rotation with Z locked as up.

        Mouse-drag rotates azimuth + elevation freely; the world
        stays vertical (no roll) because ``camera.up = (0, 0, 1)``.
        """
        fig = render_footings_3d(_make_sapatas(1))
